...
public class AdvancedFinancialControlServiceTests
{
    private readonly Mock<FinanceDbContext> _mockContext;
    private readonly AdvancedFinancialControlService _service;

    public AdvancedFinancialControlServiceTests()
    {
        _mockContext = new Mock<FinanceDbContext>();
        _service = new AdvancedFinancialControlService(_mockContext.Object);
    }
    ...
    [Fact]
    public async Task GetSpendingAuthorizationLimitAsync_ValidUser_ReturnsLimit()
    {
        // Arrange
        var userId = Guid.NewGuid();
        var companyId = Guid.NewGuid();
        ...
        // Act
        var result = await _service.GetSpendingAuthorizationLimitAsync(userId, companyId);

        // Assert
        Assert.Equal(100000, result); // Default limit
    }
    ...
    [Fact]
    public async Task MonitorUnusualTransactionPatternsAsync_NormalTransaction_ReturnsNoExceptions()
    {
        // Arrange
        var companyId = Guid.NewGuid();
        var journal = new Journal
    ...
    [Fact]
    public async Task MonitorUnusualTransactionPatternsAsync_LargeTransaction_ReturnsException()
    {
        // Arrange
        var companyId = Guid.NewGuid();
        var journal = new Journal
        ...
        Assert.Contains(result, ex => ex.Contains("significantly exceeds average"));
    }

    [Fact]
    public async Task MonitorFinancialRisksAsync_HighValueTransaction_ReturnsAlert()
    {
        // Arrange
        var companyId = Guid.NewGuid();
        var asOfDate = DateTime.Today;

        var highValueJournal = new Journal
        {
            Id = Guid.NewGuid(),
            CompanyId = companyId,
            ...
            Entries = new List<JournalEntry>
            {
                new JournalEntry
                {
                    Id = Guid.NewGuid(),
                    AccountId = Guid.NewGuid(),
                    Debit = 60000, // Above threshold of 50000
                    Credit = 0
                }
            }
        };

        var journals = new List<Journal> { highValueJournal };
        _mockContext.Setup(c => c.Journals.Where(It.IsAny<System.Linq.Expressions.Expression<Func<Journal, bool>>>()))
            .Returns(journals.AsQueryable());

        // Act
        var result = await _service.MonitorFinancialRisksAsync(companyId, asOfDate);

        // Assert
        var alerts = result.ToList();
        Assert.NotEmpty(alerts);
        var highValueAlert = alerts.FirstOrDefault(a => a.AlertType == "High Value Transaction");
        Assert.NotNull(highValueAlert);
        Assert.Equal("High", highValueAlert.Severity);
    }
}
